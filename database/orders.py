...
def delete_order(conn, order_id, expected_status):
    cursor = conn.execute(
        "DELETE FROM orders WHERE id = ? AND status = ?", (order_id, expected_status)
    )
...
def get_all_orders(search_query="", status=""):
    ...
    try:
        ...
        if search_query:
            condition.append(
                "(id LIKE ? OR customer_name LIKE ? OR customer_email LIKE ? OR customer_phone LIKE ? OR customer_address LIKE ?)"
            )
