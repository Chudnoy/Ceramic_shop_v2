...
def archive_product_route(product_id):
    ...
    was_archived, error_message, product_name = archive_product_with_order_check(
        product_id
    )
...
def new_product():
    ...
    if request.method == "POST":
        ...
        is_valid_tags, tag_error_messages, cleaned_tag_ids = process_product_tag_ids(
            request.form
        )
        ...
        is_created, create_error, _product_id = create_product_with_tags(
            data=data, tag_ids=cleaned_tag_ids, file=file
        )
...
def update_product_state_route(product_id):
    ...
    is_updated, update_message = update_product_state_with_order_check(
        product_id=product_id, data=cleaned_data
    )
...
def delete_product_route(product_id):
    ...
    is_deleted, delete_message = delete_product_with_image(
        product_id=product_id, image_path=product["img"]
    )
