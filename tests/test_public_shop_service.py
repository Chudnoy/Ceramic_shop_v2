...
def create_test_work(conn, work_id, name, is_published=1):
    conn.execute(
        """
        INSERT INTO works
...
def create_test_shop_item(
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (shop_item_id, work_id, name, price, inventory_type, stock_quantity, is_published, is_orderable, is_retired)
    )
...
def test_get_public_shop_page_data_applies_shop_visibility_policy(empty_db, db_connection):
    conn = db_connection()
    
    create_test_work(conn, work_id="work-available", name="Доступная работа")
    create_test_shop_item(conn, shop_item_id="shop-available", work_id="work-available", inventory_type="unique", stock_quantity=1)
    
    create_test_work(conn, work_id="work-reserved", name="Зарезервированная работа")
    create_test_shop_item(conn, shop_item_id="shop-reserved", work_id="work-reserved", inventory_type="unique", stock_quantity=1)
    create_test_order(conn, order_id="order-reserved", status="new")
    create_test_order_item(conn, order_id="order-reserved", shop_item_id="shop-reserved", quantity=1)
    
    create_test_work(conn, work_id="work-sold", name="Проданная работа")
    create_test_shop_item(conn, shop_item_id="shop-sold", work_id="work-sold", inventory_type="unique", stock_quantity=0)
    
    create_test_shop_item(conn, shop_item_id="shop-stock-empty", name="Тиражная кружка", inventory_type="stock", stock_quantity=0)
    
    create_test_shop_item(conn, shop_item_id="shop-disabled", name="Отключённый товар", inventory_type="stock", stock_quantity=5, is_orderable=0)
    
    create_test_work(conn, work_id="work-hidden", name="Скрытая работа", is_published=0)
    create_test_shop_item(conn, shop_item_id="shop-hidden-work", work_id="work-hidden", inventory_type="unique", stock_quantity=1)
    
    create_test_shop_item(conn, shop_item_id="shop-retired", name="Архивный товар", inventory_type="stock", stock_quantity=5, is_retired=1)
