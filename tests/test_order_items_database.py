...
def create_test_product(conn, product_id="product-1", name="Башня", price=30000):
    conn.execute(
        """
...
def test_order_items_are_deleted_when_order_is_deleted(empty_db, db_connection):
    ...
    product_count = check_conn.execute(
        "SELECT COUNT(*) FROM products WHERE id = ?", ("product-1",)
    ).fetchone()[0]
...
def test_insert_order_items_creates_all_items(empty_db, db_connection):
    ...
    create_test_product(conn, product_id="product-1", name="Башня", price=30000)
    create_test_product(conn, product_id="product-2", name="Чаша", price=5000)
    ...
    item_ids = order_items.insert_order_items(
        conn=conn, order_id="order-1", items=items
    )
...
def test_get_order_items_by_order_id_returns_only_requested_order_items(
    ...
    create_test_order(conn=conn, order_id="order-1")
    create_test_order(conn=conn, order_id="order-2", customer_email="other@email.com")
    ...
    check_conn = db_connection()
    saved_items = order_items.get_order_items_by_order_id(
        conn=check_conn, order_id="order-1"
    )
    check_conn.close()
...
def test_update_order_item_quantity_updates_requested_item(empty_db, db_connection):
    ...
    is_updated = order_items.update_order_item_quantity(
        conn=conn, order_id="order-1", item_id=1, quantity=3
    )
