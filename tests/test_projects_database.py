...
def test_get_published_projects_filters_and_supports_sorting(empty_db, db_connection):
    ...
    create_test_project(
        conn, project_id="project-1", name="Пористые формы", slug="poristye-formy"
    )
    create_test_project(
        conn,
        project_id="project-2",
    ...
    create_test_project(
        conn, project_id="project-3", name="Черновик", slug="draft", is_published=0
    )
    ...
    assert [project["name"] for project in name_asc] == [
        "Архитектура памяти",
        "Пористые формы",
    ]
    assert [project["name"] for project in name_desc] == [
        "Пористые формы",
        "Архитектура памяти",
    ]
