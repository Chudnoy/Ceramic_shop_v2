...
def test_process_product_state_form_accepts_valid_data():
    form = {"status": "available", "is_visible": "1", "is_for_sale": "1"}
...
def test_process_product_state_form_rejects_invalid_status():
    form = {"status": "banana", "is_visible": "1"}
...
def test_process_product_state_form_rejects_featured_when_not_visible():
    form = {"status": "available", "is_for_sale": "1", "is_featured": "1"}
...
def test_process_product_form_does_not_look_up_category_when_status_invalid(
    monkeypatch,
):
    form = make_valid_product_form()
    form["status"] = "banana"
    ...
    monkeypatch.setattr(product_service, "get_category_by_id", fake_get_category_by_id)
...
def test_process_product_tag_ids_accepts_existing_tags(monkeypatch):
    form = MultiDict([("tag_ids", "1"), ("tag_ids", "3")])

    monkeypatch.setattr(
        product_service, "get_all_tags", lambda: [{"id": 1}, {"id": 2}, {"id": 3}]
    )
...
def test_process_product_tag_ids_rejects_nonexistent_tags(monkeypatch):
    form = MultiDict([("tag_ids", "1"), ("tag_ids", "5")])
...
def test_create_products_with_tag_rolls_back_when_tags_fail(
    empty_db, db_connection, monkeypatch
):

    conn = db_connection()
    conn.execute(
        "INSERT INTO categories (id, name, slug) VALUES (?, ?, ?)", (1, "Чаши", "bowls")
    )
    conn.commit()
    conn.close()
    ...
    monkeypatch.setattr(
        product_service, "save_image", lambda file: (True, "", test_image_path)
    )
    ...
    monkeypatch.setattr(product_service, "replace_product_tags", fail_to_replace_tags)
    ...
    with pytest.raises(RuntimeError, match="Ошибка сохранения тегов"):
        product_service.create_product_with_tags(
            data=data, tag_ids=[1, 2], file=object()
        )
...
def test_update_product_with_tags_rolls_back_when_tags_fail(
    empty_db, db_connection, monkeypatch
):
    ...
    monkeypatch.setattr(
        product_service, "save_image", lambda file: (True, "", new_image_path)
    )
    ...
    monkeypatch.setattr(
        product_service, "has_active_order_for_product", lambda conn, product_id: False
    )
    ...
    check_conn = db_connection()
    product_after_error = check_conn.execute(
        "SELECT name, img FROM products WHERE id = ?", (product_id,)
    ).fetchone()
    check_conn.close()
...
def test_update_product_with_tags_rejects_status_change_when_product_has_active_order(
    empty_db, db_connection, monkeypatch
):
    conn = db_connection()
    conn.execute(
    ...
    monkeypatch.setattr(
        product_service, "has_active_order_for_product", lambda conn, product_id: True
    )

    def fail_if_save_image_called(file):
        raise AssertionError("save_image не должен вызываться")

    monkeypatch.setattr(product_service, "save_image", fail_if_save_image_called)
    ...
    assert result == (
        False,
        "Нельзя изменить статус работы, связанной с активным заказом",
    )

    check_conn = db_connection()
    product_after_error = check_conn.execute(
        "SELECT name, description, price, img, status FROM products WHERE id = ?",
        ("product-1",),
    ).fetchone()
    check_conn.close()
...
def test_product_archive_action_returns_error_when_product_not_found(
    empty_db, db_connection, service_function
):
    conn = db_connection()
    conn.execute(
        "INSERT INTO categories (id, name, slug) VALUES (?, ?, ?)", (1, "Чаши", "bowls")
    )
    create_test_product(conn=conn, product_id="12345")
    conn.commit()
    conn.close()
...
def test_product_archive_action_updates_archived_state(
    empty_db, db_connection, service_function, initial_is_archived, expected_is_archived
):
    conn = db_connection()
    conn.execute(
        "INSERT INTO categories (id, name, slug) VALUES (?, ?, ?)", (1, "Чаши", "bowls")
    )
    create_test_product(conn=conn, is_archived=initial_is_archived)
    conn.commit()
    conn.close()
...
def test_archive_product_returns_error_when_product_has_active_order(
    ...
    was_archived, error_message, product_name = (
        product_service.archive_product_with_order_check("product-1")
    )
