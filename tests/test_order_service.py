...
def create_test_order_with_item(
    ...
    create_test_order(
        conn=conn, order_id=order_id, total=unit_price * quantity, status=order_status
    )
...
def make_order_item(product_id, product_name, unit_price, quantity=1):
    return {
        "product_id": product_id,
        "product_name": product_name,
        "unit_price": unit_price,
...
def test_create_order_with_items_rolls_back_when_item_insert_fails(
    empty_db, db_connection, monkeypatch
):
    conn = db_connection()
    ...
    monkeypatch.setattr(order_service, "insert_order_items", failing_insert_order_items)
    ...
    order_items_count = check_conn.execute(
...
def test_update_order_with_items_updates_order_and_all_items(empty_db, db_connection):
    ...
    create_test_product(conn=conn, product_id="product-1", name="Башня", price=30000)
    create_test_product(conn=conn, product_id="product-2", name="Чаша", price=5000)

    create_test_order(conn=conn, order_id="order-1", total=40000)
    ...
    data = {
        ...
        "address": "new address",
        "items": [
            {"id": first_item_id, "quantity": 2},
            {"id": second_item_id, "quantity": 3},
        ],
        "total": 75000,
    }

    is_updated, error_message = order_service.update_order_with_items(
        order_id="order-1", data=data
    )
...
def test_create_order_with_items_reserves_all_products(empty_db, db_connection):
    ...
    is_created, error_message, order_id = order_service.create_order_with_items(
        data=make_checkout_data(), items=items
    )

    check_conn = db_connection()
    saved_products = check_conn.execute(
...
def test_create_order_with_items_rolls_back_when_product_cannot_be_reserved(
    empty_db, db_connection
):
    conn = db_connection()
    create_test_product(
        conn=conn, product_id="product-1", name="Башня", price=30000, status="available"
    )
    create_test_product(
        conn=conn, product_id="product-2", name="Чаша", price=5000, status="reserved"
    )
    conn.commit()
    conn.close()
...
def test_cancel_order_cancels_order_and_releases_product(empty_db, db_connection):
    ...
    create_test_order_with_item(
        conn=conn, order_status="new", product_status="reserved"
    )
    conn.commit()
    conn.close()
    ...
    saved_items_count = check_conn.execute(
        "SELECT COUNT(*) FROM order_items WHERE order_id = ?", ("order-1",)
    ).fetchone()[0]
    check_conn.close()
...
def test_cancel_order_does_not_cancel_completed_order(empty_db, db_connection):
    conn = db_connection()
    create_test_order_with_item(
        conn=conn, order_status="completed", product_status="sold"
    )
    conn.commit()
    conn.close()
...
def test_cancel_order_rolls_back_when_product_cannot_be_released(
    empty_db, db_connection
):
    conn = db_connection()
    ...
    create_test_product(
        conn=conn, product_id="product-2", name="Чаша", price=5000, status="available"
    )
...
def test_delete_canceled_order_deletes_order_and_items_but_keeps_product(
    empty_db, db_connection
):
    conn = db_connection()
    create_test_order_with_item(
        conn=conn, order_status="canceled", product_status="available"
    )
    conn.commit()
    conn.close()
    ...
    check_conn = db_connection()
    orders_count = check_conn.execute(
...
def test_confirm_order_leaves_non_new_order_unchanged(
    empty_db, db_connection, order_status, product_status
):
    conn = db_connection()
    create_test_order_with_item(
        conn=conn, order_status=order_status, product_status=product_status
    )
    conn.commit()
    conn.close()
...
def test_complete_order_sets_order_to_completed_and_product_to_sold(
    ...
    conn = db_connection()
    create_test_order_with_item(
        conn=conn, order_status="confirmed", product_status="reserved"
    )
    conn.commit()
    conn.close()
...
def test_complete_order_rolls_back_when_product_cannot_be_sold(empty_db, db_connection):
    ...
    create_test_product(
        conn=conn, product_id="product-1", name="Башня", price=30000, status="reserved"
    )
    create_test_product(
        conn=conn, product_id="product-2", name="Чаша", price=5000, status="available"
    )
    create_test_order(conn=conn, order_id="order-1", total=35000, status="confirmed")
    create_test_order_item(
        conn=conn,
        order_id="order-1",
        product_id="product-1",
...
def test_update_order_with_items_does_not_update_confirmed_order(
    ...
    saved_item = conn.execute(
        "SELECT id FROM order_items WHERE order_id = ?", ("order-1",)
    ).fetchone()
    ...
    saved_item = check_conn.execute(
        "SELECT quantity FROM order_items WHERE id = ?", (item_id,)
    ).fetchone()

    check_conn.close()

    assert is_updated is False
    assert error_message == ("Заказ не найден или уже нельзя редактировать")
