...
def replace_test_product_tags(conn, product_id, tag_ids):
    ...
    tags.replace_product_tags(conn=conn, product_id=product_id, tag_ids=tag_ids)
...
def test_get_tags_for_product_returns_correct_sorted_tags(empty_db, db_connection):
    ...
    conn = db_connection()
    products = [(1, "Ваза", 1000), (2, "Чашка", 2000)]
    conn.executemany(
        "INSERT INTO products (id, name, price) VALUES (?, ?, ?)", products
    )
    conn.commit()
    conn.close()
...
def test_update_product_tags_replaces_old_tags_with_new_tags(empty_db, db_connection):
    ...
    conn = db_connection()
    conn.execute(
        "INSERT INTO products (id, name, price) VALUES (?, ?, ?)", (1, "Ваза", 1000)
    )
    conn.commit()
    ...
    conn = db_connection()
    replace_test_product_tags(
        conn,
        product["id"],
        [tags.get_tag_by_slug("wind")["id"], tags.get_tag_by_slug("house")["id"]],
    )
    conn.commit()
    conn.close()
...
def test_update_product_tags_deletes_tags_when_tags_list_is_empty(
...
def test_get_tags_with_product_count_returns_correct_product_counts(
    empty_db, db_connection
):

    conn = db_connection()
    products = [(1, "Ваза", 2000), (2, "Чашка", 3000)]
    conn.executemany(
        "INSERT INTO products (id, name, price) VALUES (?, ?, ?)", products
    )
    conn.commit()
...
def test_get_product_count_by_tag_id_returns_correct_product_counts(
    empty_db, db_connection
):

    conn = db_connection()
    products = [(1, "Ваза", 1000), (2, "Чашка", 2000), (3, "Тарелка", 3000)]
    conn.executemany(
        "INSERT INTO products (id, name, price) VALUES (?, ?, ?)", products
    )
    conn.commit()
