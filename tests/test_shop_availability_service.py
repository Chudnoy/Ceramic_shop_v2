...
def test_get_shop_item_availability_returns_available_stock_state(
    empty_db, db_connection
):
    conn = db_connection()

    create_test_shop_item(
        conn, shop_item_id="shop-1", inventory_type="stock", stock_quantity=6
    )
...
def test_get_shop_item_availability_returns_fully_reserved_stock_state(
    empty_db, db_connection
):
    conn = db_connection()

    create_test_shop_item(
        conn, shop_item_id="shop-1", inventory_type="unique", stock_quantity=1
    )
    create_test_order(conn, order_id="order-1", status="new")
...
def test_get_shop_item_availability_returns_out_of_stock_state(empty_db, db_connection):
    ...
    create_test_shop_item(
        conn, shop_item_id="shop-1", inventory_type="unique", stock_quantity=0
    )
...
def test_get_shop_item_availability_raises_error_for_over_reservation(
    empty_db, db_connection
):
    conn = db_connection()
    ...
    with pytest.raises(
        ValueError, match="reserved_quantity cannot exceed stock_quantity"
    ):
