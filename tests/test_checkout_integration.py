def test_checkout_creates_order_reserves_product_and_clears_cart(empty_db, db_connection, client):
    ...
    saved_order = check_conn.execute("SELECT id, customer_name, customer_email, total, status FROM orders").fetchone()
    saved_item = check_conn.execute("SELECT order_id, product_id, product_name, unit_price, quantity FROM order_items").fetchone()
...
def test_checkout_rejects_creating_partial_order_without_confirmation(empty_db, db_connection, client):

    conn = db_connection()
    conn.execute("INSERT INTO products (id, name, price, status) VALUES (?, ?, ?, ?)", ('product-1', 'Башня', 30000, 'available'))
    conn.execute("INSERT INTO products (id, name, price, status) VALUES (?, ?, ?, ?)", ('product-2', 'Чаша', 10000, 'sold'))
    conn.commit()
    conn.close()
...
def test_checkout_creates_partial_order_with_confirmation(empty_db, db_connection, client):
    ...
    check_conn = db_connection()
    saved_orders = check_conn.execute("SELECT id, customer_name, customer_email, total, status FROM orders").fetchall()
    saved_items = check_conn.execute("SELECT order_id, product_id, product_name, unit_price, quantity FROM order_items").fetchall()
    saved_product_rows = check_conn.execute("SELECT id, status FROM products").fetchall()
    check_conn.close()
    ...
    saved_product_statuses = {
        ...
        for product in saved_product_rows
    }
