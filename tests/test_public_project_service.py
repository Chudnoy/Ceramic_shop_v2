...
def test_get_public_project_page_data_returns_project_with_works(empty_db, db_connection):
    conn = db_connection()
    ...
    create_test_work(conn, work_id="work-1", slug="kaplya", name="Капля", project_position=1)
    create_test_work(conn, work_id="work-2", slug="kolonna", name="Колонна", project_position=2)
    create_test_work(conn, work_id="work-3", slug="belaya-chasha", name="Белая чаша", project_position=3)
    create_test_work_image(conn, work_id="work-1", image_path="static/kaplya-cover.jpg")
    create_test_work_image(conn, work_id="work-2", image_path="static/kolonna-cover.jpg")
    ...
    assert [
        (image["image_path"], image["position"])
