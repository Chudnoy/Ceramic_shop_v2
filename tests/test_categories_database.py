...
def test_delete_category_with_products_violates_foreign_key(
    empty_db,
    db_connection,
):
    conn = db_connection()
...
def test_get_all_categories_with_product_count_returns_correct_counts(
    ...
    vases_category_id = categories.create_category(
        conn=conn,
        name="Вазы",
        slug="vases",
        description="Красивые вазы",
    )
...
def test_create_category_and_get_by_slug(empty_db, db_connection):
    conn = db_connection()
    ...
    category_id = categories.create_category(
        ...
        name="Скульптура",
        slug="sculpture",
        description="Объёмные художественные работы",
    )
...
def test_get_category_by_id_returns_correct_category(
    empty_db,
    db_connection,
):
    conn = db_connection()
    ...
    cups_category_id = categories.create_category(
        conn=conn,
        name="Чашки",
        slug="cups",
        description="",
    ...
    category = categories.get_category_by_id(cups_category_id)

    assert category is not None
    assert category["id"] == cups_category_id
    assert category["name"] == "Чашки"
    assert category["slug"] == "cups"


def test_update_category_updates_fields_and_slug(empty_db, db_connection):
    conn = db_connection()
...
def test_delete_category_returns_true_and_removes_record(
    ...
    is_deleted = categories.delete_category(
        conn=conn,
        category_id=category_id,
    )
...
def test_delete_category_returns_false_for_unknown_id(
    empty_db,
    db_connection,
):
    conn = db_connection()
