...
def test_get_all_categories_with_product_count_returns_correct_counts(categories_test_db):
    ...
    correct_categories = {category["slug"]: category for category in all_categories if category["slug"] in {"vases", "cups"}}
...
def test_get_all_categories_with_product_count_returns_correct_counts_after_deleting_one_of_products(categories_test_db):
    ...
    new_vases_category = next(category for category in new_all_categories if category["slug"] == "vases")
    new_cups_category = next(category for category in new_all_categories if category["slug"] == "cups")
...
def test_create_category_and_get_by_slug(categories_test_db):
    ...
    categories.create_category(
        "Скульптура", 
        "sculpture",
        "Объёмные художественные работы"
    )
...
def test_get_category_by_id_returns_correct_category(categories_test_db):
    ...
    cups_category = next((category for category in all_categories if category["slug"] == "cups"), None)
