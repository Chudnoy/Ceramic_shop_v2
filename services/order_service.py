...
def process_order_form(form, old_items):
    ...
    for old_item in old_items:
        ...
        try:
            ...
            return (
                False,
                f"Некорректное количество товара для «{old_item['product_name']}»",
                None,
            )
...
def create_order_with_items(data, items):
    ...
    try:
        ...
        insert_order_items(conn=conn, order_id=order_id, items=items)
...
def confirm_order(order_id):
    ...
    try:
        ...
        is_order_confirmed = update_order_status(
            conn=conn, order_id=order_id, new_status="confirmed", expected_status="new"
        )
...
def complete_order(order_id):
    ...
    try:
        conn = get_db_connection()
        items = get_order_items_by_order_id(conn=conn, order_id=order_id)
        ...
        for item in items:
            ...
            if not is_sold:
                ...
                return (
                    False,
                    f"Не удалось изменить статус работы «{item['product_name']}»",
                )
