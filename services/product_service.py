...
def process_product_form(form):
    ...
    is_valid, error_message, cleaned_data = validate_product(
        name,
        price,
        description,
        category_id,
...
def process_product_state_form(form):
    ...
    if not is_visible and is_featured:
        return (
            False,
            "Чтобы оказаться на главной, работа должна отображаться на сайте",
            None,
        )
...
def create_product_with_tags(data, tag_ids, file):
    ...
    try:
        ...
        replace_product_tags(conn=conn, product_id=product_id, tag_ids=tag_ids)
        conn.commit()
...
def update_product_with_tags(product_id, old_image_path, data, tag_ids, file):
    ...
    try:
        ...
        has_active_order = has_active_order_for_product(
            conn=conn, product_id=product_id
        )
